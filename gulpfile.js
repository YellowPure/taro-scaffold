...
gulp.task('upload-cdn', async function () {
  ...
  const filePath = path.resolve('./cdn/dist');
  await addFileToOSSSync(filePath, '', files);
  const p = Async.queue(function (task, callback) {
...
gulp.task('mini', async function () {
  cleanCDNDist();
  gulp.src('cdn/src/*').pipe(imagemin()).pipe(rev()).pipe(gulp.dest('cdn/dist'));
});
...
gulp.task(
  'build',
  gulp.series(cleanBuild, gulp.parallel(minifyJS, minifyCSS, minifyHTML, minifyImg, copyOther), cleanDist, carry, wait)
);
