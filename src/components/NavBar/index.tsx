...
import { getSystemInfo, isFunction } from '@src/utils/utils';
import './style.scss';
...
> {
  ...
  setStyle(systemInfo) {
    ...
    const navigationbarinnerStyle = [
      ...
      `height:${Number(navBarHeight) + Number(navBarExtendHeight)}px`,
    ...
    return (
      <View
        ...
        style={`background: ${backgroundColorTop || background};height:${
          Number(navBarHeight) + Number(navBarExtendHeight)
        }px;`}
      >
        <View
          className={`lxy-nav-bar__placeholder ${ios ? 'ios' : 'android'}`}
          style={`padding-top: ${Number(navBarHeight) + Number(navBarExtendHeight)}px;`}
        />
        <View
          className={`lxy-nav-bar__inner ${customBgClass} ${ios ? 'ios' : 'android'}`}
          style={`${customBgClass || `background:${background}`};${navigationbarinnerStyle};`}
        >
