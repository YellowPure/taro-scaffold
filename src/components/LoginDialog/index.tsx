...
const NormalDialog = observer(({ setDialogType, setLoginType }: { setDialogType, setLoginType }) => {
  return <View className='login-dialog-normal'>
    <View className='login-dialog-tbox'>
      <Text className='login-dialog-txt'>为更有效帮您管理数据，建议进行登录</Text>
    </View>
    <View>
      <Button className='login-dialog-button' onClick={() => {
        setDialogType(2);
        setLoginType(1);
      }}
      >手机号登录</Button>
...
const PhoneDialog = observer(({ loginType, setDialogType }: { setDialogType, loginType }) => {
  ...
  const nextStep = () => {
    ...
    user.getVerifyCode(loginDialogUI.phoneValue)
      .then(res => {
        if (+res.error_code === 0) {
          setDialogType(3)
        } else {
  ...
  return <View>
    ...
    <View>
      <Input
        ...
        className='login-dialog-input black-85 font-size-16 login-input-phone' placeholder='请输入手机号'
        cursorSpacing={80}
      />
      <Button loading={user.fetching === FETCH_STATUS.Pending} className='login-dialog-button' onClick={nextStep}>获取验证码</Button>
    </View>
  </View>
})

const VerifyDialog = observer((props) => {
  ...
  async function login() {
    ...
    if (+res.error_code === 0) {
      ...
      // 登录成功后跳转到首页
      user.userinfoFetching = FETCH_STATUS.Pending;
  ...
  return <View>
    <View className='login-dialog-tbox'><Text className='login-dialog-txt'>短信验证码已发送到：
    ...
    <View>
      <View className='login-dialog-input-box'>
        <Input
          ref={codeRef}
          type='number'
          className='login-dialog-input' placeholder='请输入验证码'
          placeholderStyle='color:rgba(0,0,0,.25)'
          cursorSpacing={80}
        />
        <Text className='login-dialog-resend font-size-16' onClick={resetCode}>
          重发 {countdown > 0 ? `(${countdown})` : ''}</Text>
      </View>
      <Button loading={user.fetching === FETCH_STATUS.Pending} onClick={login} className='login-dialog-button'>确定</Button>
    </View>
  </View>
})
...
const LoginDialog = observer(() => {
  ...
  return <AtFloatLayout onClose={() => {
    Taro.showTabBar();
    loginDialogUI.visible = false;
  }} isOpened={loginDialogUI.visible} className='login-dialog'
  >
    <View className='at-row login-dialog-bar'>
      {dialogType > 1 ? <AtIcon className='login-dialog-back' value='chevron-left' size='22' onClick={() => {
        setDialogType(dialogType - 1);
      }}
      ></AtIcon> : null}
      <View className='at-col login-title'>{title}</View>
      <AtIcon className='login-close' value='close' size='22' onClick={() => {
        loginDialogUI.visible = false;
        Taro.showTabBar();
      }}
      ></AtIcon>
    ...
    <View className='login-dialog-content'>
      {dialogType === 1 ?
        ...
        : dialogType === 2 ? <PhoneDialog loginType={loginType} setDialogType={setDialogType} /> :
          <VerifyDialog />
      }
    </View>
  </AtFloatLayout>;
